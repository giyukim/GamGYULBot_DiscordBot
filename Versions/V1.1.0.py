...
statcomm = ['핑', '도움말', '청소', '김태균', '감귤', '귤', '귤아', '배워', '[질문]', '데이터베이스', '금지', '웹페이지', '디스코드', '주사위', '골라줘', '찾아']
...
async def on_message(message):
    ...
    if msg[0] == '귤아':
        ...
        if len(msg) == 1: #귤아
            ...
            if msg[1] == '도움말': #귤아 도움말
                embed = discord.Embed(title=":tangerine: 도움말 :tangerine:", description="감귤봇 V1.1.0 \n개발자: 감귤 \n문의: 감귤#1936 1jgg1020@gmail.com \n접두어: '귤아 '", color=0xffa600)
                ...
                embed.add_field(name="대화", value="배워, 찾아, [질문]", inline=True)
                ...
                if len(msg) == 2:
                    await ctx.send(embed = discord.Embed(description = '도움말:: 귤아 골라줘 <리스트1> <리스트2> ... <리스트n>'))
                else:
                ...
                if len(msg) == 4:
                    if int(msg[2]) < int(msg[3]):
                        ...
                        await ctx.send(embed = discord.Embed(description = '주사위[{} ~ {}] :: {}'.format(msg[2], msg[3], str(answer_value))))
                    elif int(msg[2]) > int(msg[3]):
                        answer_value = random.randrange(int(msg[3]), int(msg[2]) + 1)
                        await ctx.send(embed = discord.Embed(description = '주사위[{} ~ {}] :: {}'.format(msg[3], msg[2], str(answer_value))))
                    elif int(msg[2]) == int(msg[3]):
                        await ctx.send(embed = discord.Embed(description = '주사위[{}] :: {}'.format(msg[2], msg[2])))
                else:
                    await ctx.send(embed = discord.Embed(description = '도움말:: 귤아 주사위 <정수> <정수>'))
                ...
                if len(msg) == 3:
                    if msg[2] in statcomm:
                        await ctx.send(embed = discord.Embed(description = '지정된 명령어는 찾을 수 없어요'))
                    else:
                        result = sql_search(msg[2])
                        if result == '0':
                            await ctx.send(embed = discord.Embed(description = result))
                        else:
                            embed = discord.Embed(title="질문 찾기")
